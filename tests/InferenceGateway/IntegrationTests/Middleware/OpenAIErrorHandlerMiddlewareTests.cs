// <copyright file="OpenAIErrorHandlerMiddlewareTests.cs" company="PlaceholderCompany">
...
namespace Synaxis.InferenceGateway.IntegrationTests.Middleware
{
    public class OpenAIErrorHandlerMiddlewareTests
    {
        ...
        public async Task InvokeAsync_ArgumentException_Returns400WithInvalidRequestError()
        {
            // Arrange
            var context = CreateMockHttpContext();
            var exception = new ArgumentException("Invalid parameter", string.Empty);
        ...
        public async Task InvokeAsync_AggregateException_WithHttpRequestException_Returns502WithDetails()
        {
            ...
            Assert.NotNull(errorResponse);
            Assert.NotNull(errorResponse.Error);
            Assert.Contains("Routing failed", errorResponse.Error.Message, StringComparison.Ordinal);
        ...
        [Fact]
        public Task InvokeAsync_ResponseHasStarted_ThrowsException()
        {
            // Arrange
            var context = CreateMockHttpContext();
            ...
            // Act & Assert
            return Assert.ThrowsAsync<InvalidOperationException>(() => this._middleware.InvokeAsync(context));
        }
        ...
        public async Task InvokeAsync_WithRequestId_IncludesRequestIdInError()
        {
            ...
            var exception = new ArgumentException("Test error", string.Empty);
            this._mockNext.Setup(next => next(It.IsAny<HttpContext>())).ThrowsAsync(exception);
            ...
            Assert.Equal((int)HttpStatusCode.BadRequest, context.Response.StatusCode);
            var responseBody = await GetResponseBody(context);
            Assert.Contains("Test error", responseBody, StringComparison.Ordinal);
        }
        ...
        public async Task InvokeAsync_StreamRequest_HandlesStreamingError()
        {
            ...
            var responseBody = await GetResponseBody(context);
            Assert.Contains("data: {", responseBody, StringComparison.Ordinal);
            Assert.Contains("data: [DONE]", responseBody, StringComparison.Ordinal);
        }
        ...
        public async Task InvokeAsync_InvalidModelException_Returns404()
        {
            // Arrange
            var context = CreateMockHttpContext();
            var exception = new ArgumentException("Model not found", string.Empty);
            ...
            // Assert
            var responseBody = await GetResponseBody(context);
            Assert.Contains("Model not found", responseBody, StringComparison.Ordinal);
        }
        ...
        public async Task InvokeAsync_OpenAIFormat_ReturnsOpenAICompatibleError()
        {
            // Arrange
            var context = CreateMockHttpContext();
            var exception = new ArgumentException("Invalid request parameters", string.Empty);
            this._mockNext.Setup(next => next(It.IsAny<HttpContext>())).ThrowsAsync(exception);
...
#pragma warning restore S4144 // Methods should not have identical implementations
        ...
        public async Task InvokeAsync_StreamingRequestWithStreamQuery_ReturnsSSEFormatError()
        {
            // Arrange
            var context = CreateMockHttpContext();
            context.Request.Query = new QueryCollection(new Dictionary<string, StringValues>(StringComparer.Ordinal)
        {
            { "stream", "true" },
        });
            var exception = new InvalidOperationException("Stream error");

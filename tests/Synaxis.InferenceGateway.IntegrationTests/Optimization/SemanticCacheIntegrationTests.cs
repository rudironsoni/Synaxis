...
public class SemanticCacheIntegrationTests(ITestOutputHelper output, QdrantFixture qdrantFixture)
{
    private readonly ITestOutputHelper _output = output ?? throw new ArgumentNullException(nameof(output));
    private readonly QdrantFixture _qdrantFixture = qdrantFixture ?? throw new ArgumentNullException(nameof(qdrantFixture));

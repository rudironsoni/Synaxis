...
public class SmartRouterTests
{
    ...
    public async Task GetCandidatesAsync_ShouldSortByPriority_FreeFirst_ThenCost_ThenTier()
    {
        ...
        this._mockRoutingScoreCalculator
            .Setup(x => x.CalculateScoreAsync(It.Is<EnrichedCandidate>(c => c.Key == "free-tier1"), null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(100.0); // Free + tier 1
        this._mockRoutingScoreCalculator
            .Setup(x => x.CalculateScoreAsync(It.Is<EnrichedCandidate>(c => c.Key == "free-tier2"), null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(90.0); // Free + tier 2
        this._mockRoutingScoreCalculator
            .Setup(x => x.CalculateScoreAsync(It.Is<EnrichedCandidate>(c => c.Key == "paid-tier2"), null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(50.0); // Paid + lower cost
        this._mockRoutingScoreCalculator
            .Setup(x => x.CalculateScoreAsync(It.Is<EnrichedCandidate>(c => c.Key == "paid-tier1"), null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(40.0); // Paid + higher cost

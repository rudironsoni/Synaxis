...
public class SynaxisWebApplicationFactory : WebApplicationFactory<Program>, ITestOutputHelperAccessor
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((context, config) =>
        {
            ...
            if (!string.IsNullOrEmpty(groqKey)) overrides["Synaxis:Providers:Groq:Key"] = groqKey;

            var cohereKey = Environment.GetEnvironmentVariable("SYNAPLEXER_COHERE_KEY");
            if (!string.IsNullOrEmpty(cohereKey)) overrides["Synaxis:Providers:Cohere:Key"] = cohereKey;

            var cfKey = Environment.GetEnvironmentVariable("SYNAPLEXER_CLOUDFLARE_KEY");
            if (!string.IsNullOrEmpty(cfKey)) overrides["Synaxis:Providers:Cloudflare:Key"] = cfKey;

            var cfAccount = Environment.GetEnvironmentVariable("SYNAPLEXER_CLOUDFLARE_ACCOUNT_ID");
            if (!string.IsNullOrEmpty(cfAccount)) overrides["Synaxis:Providers:Cloudflare:AccountId"] = cfAccount;

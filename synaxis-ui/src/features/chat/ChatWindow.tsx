...
function useChat(sessionId?: number){
  ...
  const send = async (text:string)=>{
    if(!sessionId) return
    const now = new Date()
    const userMsg = { sessionId, role: 'user', content: text, createdAt: now }
    ...
    try{
      ...
      const reply = { sessionId, role: 'assistant', content: assistantContent, createdAt: new Date(), tokenUsage: usage }
      const rid = await db.messages.add(reply as any)

// <copyright file="ApplicationHealthCheck.cs" company="Synaxis">
    ...
    public class ApplicationHealthCheck : IHealthCheck
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                CheckThreadPool(data, issues);
                CheckGarbageCollection(data, issues);
                CheckProcess(data, issues);
                AddApplicationInfo(data);

                // Determine health status
                if (issues.Count > 0)
                {
                    this._logger.LogWarning("Application health check degraded: {Issues}", string.Join(", ", issues));
                ...
                this._logger.LogInformation("Application health check passed");
                return Task.FromResult(HealthCheckResult.Healthy("Application is healthy", data));
            }
            catch (Exception ex)
            {
                this._logger.LogError(ex, "Application health check failed");
                return Task.FromResult(HealthCheckResult.Unhealthy("Application health check failed", exception: ex));
            }
        }

        private static void CheckThreadPool(Dictionary<string, object> data, List<string> issues)
        {
        ...
        private static (int WorkerThreads, int CompletionPortThreads, int MinWorkerThreads, int MinCompletionPortThreads) CheckThreadPool()
        {
        ...
        private static void CheckGarbageCollection(Dictionary<string, object> data, List<string> issues)
        {
        ...
        private static (int Gen0Collections, int Gen1Collections, int Gen2Collections, double TotalMemoryMB, double PauseTimeMs) CheckGarbageCollection()
        {
        ...
        private static void CheckProcess(Dictionary<string, object> data, List<string> issues)
        {
        ...
        private static (int ProcessId, double CpuTimeMs, double WorkingSetMB, double PrivateMemoryMB, int HandleCount, int ThreadCount) CheckProcess()
        {
        ...
        private static void AddApplicationInfo(Dictionary<string, object> data)
        {

...
namespace Synaxis.Infrastructure.EventSourcing.Publishing;
...
public sealed class DomainEventPublisher : IDomainEventPublisher
{
    ...
    public async Task PublishAsync<TEvent>(
        ...
        where TEvent : Synaxis.Abstractions.Commands.INotification
    {
        if (@event is null)
        {
    ...
    public async Task PublishAllAsync(
        IEnumerable<Synaxis.Abstractions.Commands.INotification> events,
        CancellationToken cancellationToken = default)
    {
        if (events is null)
        {

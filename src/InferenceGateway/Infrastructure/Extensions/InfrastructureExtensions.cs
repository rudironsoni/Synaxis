        ...
        public static IServiceCollection AddSynaxisInfrastructure(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (synaxisConfig != null)
            {
                foreach (var provider in synaxisConfig.Providers)
                {
                    ...
                    {
                        case "openai":
                            services.AddOpenAiCompatibleClient(name, config.Endpoint ?? "https://api.openai.com/v1", config.Key ?? "", defaultModel, config.CustomHeaders != null ? new Dictionary<string, string>(config.CustomHeaders) : null);
                            break;
                        case "groq":
                            services.AddOpenAiCompatibleClient(name, "https://api.groq.com/openai/v1", config.Key ?? "", defaultModel, config.CustomHeaders != null ? new Dictionary<string, string>(config.CustomHeaders) : null);
                            break;
                        case "cohere":
                        ...
                        default:
                            ...
                            {
                                services.AddOpenAiCompatibleClient(name, config.Endpoint, config.Key ?? "", defaultModel, config.CustomHeaders != null ? new Dictionary<string, string>(config.CustomHeaders) : null);
                            }
                            break;
                    }
                }

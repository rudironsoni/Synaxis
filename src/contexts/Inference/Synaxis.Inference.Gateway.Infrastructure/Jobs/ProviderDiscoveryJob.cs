...
namespace Synaxis.InferenceGateway.Infrastructure.Jobs
{
    ...
    public class ProviderDiscoveryJob : IJob
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ProviderDiscoveryJob"/> class.
        /// </summary>
        ...
        private static string GetEffectiveBaseUrl(ProviderConfig config)
        {
            if (config == null)
            {
                return string.Empty;
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            {
                await this.ProcessProviderAsync(kv.Key, kv.Value, db, discovery).ConfigureAwait(false);
            }
        }
            ...
            IOpenAiModelDiscoveryClient discovery)
        {
            ...
            var baseUrl = GetEffectiveBaseUrl(providerCfg);
            ...
            try
            {
                ...
                await this.ProcessDiscoveredModelsAsync(providerKey, discoveredModels, db).ConfigureAwait(false);
            ...
            ControlPlaneDbContext db)
        {
            foreach (var found in discoveredModels)
            {
                ...
                var global = await this.EnsureGlobalModelExistsAsync(found, db).ConfigureAwait(false);
                if (global != null)
                {
                    await this.UpsertProviderModelAsync(providerKey, found, global.Id, db).ConfigureAwait(false);
                }
            }
        }
        ...
        private async Task<GlobalModel?> EnsureGlobalModelExistsAsync(string modelId, ControlPlaneDbContext db)
        {
            ...
            return await this.CreateGlobalModelAsync(modelId, db).ConfigureAwait(false);
        }
        ...
        private Task UpsertProviderModelAsync(
            string providerKey,
            string modelId,
            ...
            ControlPlaneDbContext db)
        {
            ...
            if (existing == null)
            {
                return this.CreateProviderModelAsync(providerKey, modelId, globalModelId, db);
            }
            else
            {
                return UpdateProviderModelAsync(existing, globalModelId, db);
            }
        }
        ...
        private static async Task UpdateProviderModelAsync(
